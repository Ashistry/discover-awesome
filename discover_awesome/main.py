...
class DiscoverAwesome:
	...
	def showResults(self,path,limit=10):
		...
		# Create a list of options based on the entries in the JSON data
		options = []
		for entry in data[:limit]:  # Limit the number of entries to 'limit'
		    options.append(entry['name'])  # Assuming each entry has a 'name' key
          # Check if there are any options to display
		if not options:
			print("No entries found.")
			return

		# Create and show the terminal menu
